...
# Scenario 1 - Measurement: Sleep Score, ATE: 5, SD: 5

For this scenario we assume:

-   The average non-intervention sleep score is `r control_avg_score` (for scenarios 1, 2, & 3). This average is based off of data provided by Garmin (\textit{Garmin Blog}).
-   True $ATE_{sc1} = 5$. This is based on a small trial run of our experiment conducted with ourselves.
-   $\sigma_{sc1}$ = 5, regardless of intervention
...
# Scenario 2 - Measurement: Sleep Score, ATE: 3, SD: 3

For this scenario we assume:

-   The average non-intervention sleep score is `r control_avg_score`.
-   True $ATE_{sc2}$ = 3
-   $\sigma_{sc2}$ = 3, regardless of intervention
...
# Scenario 3 - Measurement: Sleep Score, ATE: 5, SD: 7

For this scenario we assume:

-   The average non-intervention sleep score is `r control_avg_score`.
-   True $ATE_{sc3}$ = 5
-   $\sigma_{sc3}$ = 7, regardless of intervention
...
# Scenario 5 - Deep & REM sleep meaningful ATEs
...
```{r echo=FALSE, include=TRUE}
#list of data.tables
data_tables <- list(ate_5, ate_3, ate_5_7, ate_rem, ate_rem_o)
