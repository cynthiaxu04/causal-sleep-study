...
We simulate the experiment in different scenarios of true average treatment effect (ATE) and variance. For each scenario, we calculate power at different sample sizes, from 10 to 100 subjects. We run 1,000 simulations at each sample size, using a two-tailed t-test for hypothesis testing. Note: Because we are running a paired test, each subject has a control and treatment value; for example, 10 subjects yields 10 control and 10 treatment sleep measurements.
...
-   Sleep scores are normally distributed in the population.
...
```{r echo=FALSE, include=TRUE}
#list of data.tables
data_tables <- list(ate_s1, ate_s2, ate_s3, ate_s4, ate_rem)
