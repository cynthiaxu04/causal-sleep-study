...
mean_ctrl <- mean(data$sleep_score[data$treat == 0])
mean_treat <- mean(data$sleep_score[data$treat == 1])


hist_ctrl <-  ggplot() + 
  geom_histogram(
    aes(data[treat == 0, avg_ss]),
    fill = "#0099F8",
    color="black",
    breaks = seq(50,100,5),
    alpha = 0.6) +
  geom_vline(xintercept = mean_ctrl, color = "red", linetype = "dashed", size = 1) + 
  labs(
...
```{r lineplot, include=TRUE, warning=FALSE, render=FALSE, echo=FALSE, error=FALSE, message=FALSE}
